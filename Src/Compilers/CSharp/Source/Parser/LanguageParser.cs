...
namespace Microsoft.CodeAnalysis.CSharp.Syntax.InternalSyntax
{
    internal class LanguageParser : SyntaxParser
    {
        ...
        private static SyntaxModifier GetModifier(SyntaxToken token)
        {
            switch (token.Kind)
            {
                ...
                case SyntaxKind.ImplicitKeyword:
                    return SyntaxModifier.Implicit;
                case SyntaxKind.IdentifierToken:
        ...
        private void ParseModifiers(SyntaxListBuilder tokens)
        {
            ...
            while (true)
            {
                ...
                switch (newMod)
                {
                    ...
                    case SyntaxModifier.Implicit:
                        {
                            if (this.PeekToken(1).Kind != SyntaxKind.OperatorKeyword)
                            {
                                goto default;
                            }
                            else
                            {
                                // "implicit operator" means no more modifiers
                                return;
                            }
                        }
        ...
        private MemberDeclarationSyntax ParseMemberDeclarationOrStatement(SyntaxKind parentKind, string typeName = null)
        {
            ...
            try
            {
                ...
                if (this.CurrentToken.Kind == SyntaxKind.ExplicitKeyword ||
                    (this.CurrentToken.Kind == SyntaxKind.ImplicitKeyword && this.PeekToken(1).Kind == SyntaxKind.OperatorKeyword) ||
        ...
        private ConversionOperatorDeclarationSyntax ParseConversionOperatorDeclaration(SyntaxListBuilder<AttributeListSyntax> attributes, SyntaxListBuilder modifiers)
        {
            ...
            return syntaxFactory.ConversionOperatorDeclaration(attributes, modifiers.ToTokenList(), style, opKeyword, type, paramList, body, semicolon);
        }
            ...
            TypeSyntax type)
        {
            ...
            return syntaxFactory.OperatorDeclaration(attributes, modifiers.ToTokenList(), type, opKeyword, opToken, paramList, body, semicolon);
        }
        ...
        private void ParseParameterModifiers(SyntaxListBuilder modifiers, bool allowThisKeyword, bool allowFieldModifiers)
        {
                ...
                if (IsParameterModifier(this.CurrentToken.Kind, allowThisKeyword))
            {
                ...
                if (mod.Kind == SyntaxKind.ThisKeyword ||
                    ...
                    mod.Kind == SyntaxKind.ImplicitKeyword)
                {
                        if (mod.Kind == SyntaxKind.ThisKeyword)
                        {
                            ...
                            else if ((flags & (ParamFlags.Params | ParamFlags.Implicit)) != 0)
                            {
                        ...
                        else if (mod.Kind == SyntaxKind.RefKeyword)
                        {
                            ...
                            else if ((flags & (ParamFlags.Out | ParamFlags.Implicit)) != 0)
                            {
                        ...
                        else if (mod.Kind == SyntaxKind.OutKeyword)
                        {
                            ...
                            else if ((flags & (ParamFlags.Ref | ParamFlags.Implicit)) != 0)
                            {
                                mod = this.AddError(mod, ErrorCode.ERR_MultiParamMod);
                            }
                            else
                        ...
                        else if (mod.Kind == SyntaxKind.ParamsKeyword)
                        {
                            ...
                            else if ((flags & (ParamFlags.Ref | ParamFlags.Out | ParamFlags.This | ParamFlags.Implicit)) != 0)
                            {
                                mod = this.AddError(mod, ErrorCode.ERR_MultiParamMod);
                            }
                            else
                        ...
                        else if (mod.Kind == SyntaxKind.ImplicitKeyword)
                        {
                            if ((flags & ParamFlags.Implicit) != 0)
                            {
                                mod = this.AddError(mod, ErrorCode.ERR_DupParamMod, SyntaxFacts.GetText(SyntaxKind.ImplicitKeyword));
                            }
                            else if ((flags & ParamFlags.This) != 0)
                            {
                                mod = this.AddError(mod, ErrorCode.ERR_BadParamModThis);
                            }
                            else if ((flags & (ParamFlags.Ref | ParamFlags.Out | ParamFlags.Params)) != 0)
                            {
                                mod = this.AddError(mod, ErrorCode.ERR_MultiParamMod);
                            }
                            else
                            {
                                flags |= ParamFlags.Implicit;
                            }
                        }
                }
        ...
        private ExpressionSyntax ParseSubExpression(uint precedence, bool allowDeclarationExpressionAtTheBeginning = true, bool contextRequiresVariable = false)
        {
            ...
            while (true)
            {
                ...
                {
                    leftOperand = syntaxFactory.BinaryExpression(opKind, leftOperand, opToken, this.ParseSubExpression(newPrecedence));
                }
            }
            ...
            while (true)
            {
                ...
                switch (tk)
                {
                    ...
                    case SyntaxKind.MinusGreaterThanToken:
                        ...
                        break;
                    case SyntaxKind.DotToken:
                        var dot = this.EatToken();
                        if (this.CurrentToken.Kind == SyntaxKind.DollarToken)
                        {
                            expr = syntaxFactory.IndexedMemberAccessExpression(expr, dot, ParseIndexedName());
                        }
                        else
                        {
                            expr = syntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, expr, dot, this.ParseSimpleName(NameOptions.InExpression));
                        }
                        break;
        ...
        private IndexedNameSyntax ParseIndexedName()
        {
            var dollar = EatToken();
            var identifier = ParseIdentifierToken();
            var indexedName = syntaxFactory.IndexedName(dollar, identifier);
            return indexedName;
        }
        ...
        private bool IsInitializerMember()
        {
            return this.IsComplexElementInitializer() || 
                ...
                this.IsIndexedMemberInitializer() ||
        ...
        private bool IsIndexedMemberInitializer()
        {
            return this.CurrentToken.Kind == SyntaxKind.DollarToken;
        }
        ...
        private ExpressionSyntax ParseObjectOrCollectionInitializerMember(ref bool isObjectInitializer)
        {
            ...
            else if (IsIndexedMemberInitializer())
            {
                isObjectInitializer = true;
                return this.ParseIndexedMemberInitializer();
            }
        ...
        private ExpressionSyntax ParseIndexedMemberInitializer()
        {
            var indexedName = this.ParseIndexedName();
            var equal = this.EatToken(SyntaxKind.EqualsToken);
            ExpressionSyntax expression;
            if (this.CurrentToken.Kind == SyntaxKind.OpenBraceToken)
            {
            ...
            return syntaxFactory.BinaryExpression(SyntaxKind.SimpleAssignmentExpression, indexedName, equal, expression);
        }
